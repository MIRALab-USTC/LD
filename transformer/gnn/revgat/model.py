...
import copy
from bert.bert_model import BaseClassifier
from bert.bert_model import NodeClassifier
from ..gamlp.gamlp_utils import dgl_neighbor_average_features


MAX_N_LAYERS = 99999


class REVGATBertNodeClassifier(NodeClassifier):
    ...
    def gnn_inference(self, n_id_in, n_id_out, graph, cls_token_emb=None, lm_batch_id=None):
        gnn_x = self.hist_emb.pull()
        gnn_x = self.dropout(gnn_x)
        if cls_token_emb is not None and lm_batch_id is not None:
            gnn_x[lm_batch_id] = cls_token_emb

        logits = self.gnn_model(graph, gnn_x)

        batch_size = logits.shape[0]
        labels = graph.ndata['labels'][:batch_size].to(logits.device)
        train_mask = graph.ndata['train_mask'][:batch_size].to(logits.device)

        all_train_logits = logits[train_mask]
        all_train_labels = labels[train_mask]

        # sample_mask = torch.rand(all_train_logits.shape[0]) < 0.5
        # loss = self.loss_func(all_train_logits[sample_mask], all_train_labels[sample_mask])
        loss = self.loss_func(all_train_logits, all_train_labels)
...
class GATConv(nn.Module):
    def __init__(
        ...
        # self.fc_adjs = nn.Linear(num_heads * out_feats, num_heads * out_feats)
        # self.fc_adjs2 = nn.Linear(num_heads * out_feats, num_heads * out_feats)
...
class RevGATBlock(nn.Module):
    ...
    def pure_lin(self, feat_src, mask):

        out = self.norm(feat_src)
        out = F.relu(out, inplace=True)
        if isinstance(self.dropout, SharedDropout):
            self.dropout.set_mask(mask)
        out = self.dropout(out)
        out = self.conv.pure_lin(out)
        return out
...
class RevGAT(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        self.perms[0] = self.perms[0].to(h.device)
