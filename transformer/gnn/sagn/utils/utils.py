...
@torch.no_grad()
def gen_output_torch(model, feats, hist_emb, test_loader, device, label_emb, ema=None):
    if ema != None:
        ema.apply_shadow()
    model.eval()
    preds = []
    for batch in test_loader:
        ...
        # batch_feats = [torch.from_numpy(np.array(feats)[batch]).to(torch.float32).to(device)]
        batch_feats = [feats[i][batch].to(torch.float32).to(device) for i in range(len(feats))]
        cls_token_emb = hist_emb.pull(batch.cpu()).to(device)
